...
class SaleOrder(models.Model):
    ...
    def action_button_confirm(self):
        serviceOrderObj = self.env['service.order']
        serviceOrderLineObj = self.env['service.order.line']
        hrObj = self.env['hr.employee']
        ...
        for order in self:
            ...
            if suppliers:
                for order, service_class, supplier in suppliers.keys():
                    vals = serviceOrderObj.default_get(serviceOrderObj.fields_get().keys())
                    user_ids = hrObj.search([('user_id','=',self.env.uid)], limit=1)
                    vals.update({
                    ...
                    newServiceOrder = serviceOrderObj.create(vals)
                    for line in suppliers[(order, service_class, supplier)]:
                        vals = serviceOrderLineObj.default_get(serviceOrderLineObj.fields_get().keys())
                        vals.update(line)
