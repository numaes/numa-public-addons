...
`_prepare_product_base_line_for_taxes_computation`, the hook Odoo 20's tax engine
...
`_get_fields_onchange_balance_model` had already been removed in an earlier pass:
both went with the accounting rework several versions ago, and both called a `super()`
that was not there.
