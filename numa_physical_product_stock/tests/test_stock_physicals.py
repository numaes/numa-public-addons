...
class TestStockPhysicals(TransactionCase):
    ...
    def test_01_a_move_line_can_be_created_at_all(self):
        """A line created in code opens at the catalogue's figures.

        The totals used to be filled by a `create` override calling an onchange by
        hand; they are computed fields now, so every creation path gets them.
        """
    ...
    def test_14_a_picking_follows_a_correction(self):
        """It depended on `move_line_ids` alone, so a corrected weight on a line
        left the picking's figure where it was until something else touched it."""
        picking = self._picking()
        move = self.env['stock.move'].create({
            'name': 'Slab',
            'product_id': self._product().id,
