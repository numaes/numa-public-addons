...
TREE_TEMPLATE = '<table style="width: 100%%; border: none;" title="Conversation">' \
                '<tbody>' \
                '<tr>' \
                '<td style="width: 1%%;"><img class="rounded-circle" style="height: auto; width: 64px; padding:10px;" src="data:image/png;base64, %s" alt="Avatar" title="%s" width="100" border="0" /></td>' \
                '<td style="width: 99%%;">' \
...
class Conversation(models.Model):
    ...
    author_id = fields.Many2one(string="Author", comodel_name='res.partner', ondelete='set null',
                                default=lambda self: self.env.user.partner_id.id)
    partner_ids = fields.Many2many(string="Participants", comodel_name='res.partner',
                                   default=_default_participants)
    last_message_post = fields.Datetime(string="Last Message")
    last_message_by = fields.Many2one(string="Last Message", comodel_name='res.partner', ondelete='set null')
    is_participant = fields.Boolean(string="I participate", compute='_is_participant')

    subject_display = fields.Html(string="Subject", compute="_subject_display")
    message_count = fields.Integer(string="Messages", compute='_count_messages')
    message_needaction_count = fields.Integer(string="Messages", compute='_count_messages')
    ...
    def _count_messages(self):
        for rec in self:
            ...
            rec.update({'message_count': message_count, 'message_needaction_count': message_needaction_count})
    ...
    def create(self, vals):
        # Set current user as author if not defined. Use current date as firs message post
    ...
    # -- Write
    """
    Use 'skip_followers_test=True' in context to skip checking for followers/participants
    """
    ...
    def write(self, vals):
        ...
        for rec in self:
            ...
            # New followers added?
            followers_add = [partner.id for partner in rec.partner_ids if partner not in rec.message_partner_ids]
            if len(followers_add) > 0:
                rec.message_subscribe(partner_ids=followers_add)

            # Existing followers removed?
            followers_remove = [partner.id for partner in rec.message_partner_ids if partner not in rec.partner_ids]
            if len(followers_remove) > 0:
                rec.message_unsubscribe(partner_ids=followers_remove)

        return res

    # -- Check if partner has access to Conversations or partner is external user or not a user at all
    def has_conversations(self, partner):
        ...
        :return: 1 - has access to Conversations, 2 - is not an internal User, False - none of above
        """
    ...
    def partner_by_email(self, email_addresses):
        """
        Override this method to implement custom search (e.g. if using prt_phone_numbers module)
        :param list email_addresses: List of email addresses
        :return: res.partner obj if found.
        Please pay attention to the fact that only the first (newest) partner found is returned!
        """
        # Use loop with '=ilike' to resolve MyEmail@GMail.com cases
        for address in email_addresses:
            partner = self.env['res.partner'].search([('email', '=ilike', address)], limit=1, order='id desc')
            if len(partner) == 1:
                return partner
    ...
    def message_new(self, msg_dict, custom_values=None):
        ...
        if not author_id:
            ...
            author_id = partner.id if partner else self.env['res.partner']. \
                create({'name': partner_name if partner_name and len(partner_name) > 0 else email_address.split("@")[0],
                        'email': email_address,
                        'category_id': [(4, self.env.ref('prt_mail_messages.cetmix_conversations_partner_cat').id)]
                        if self.env.ref('prt_mail_messages.cetmix_conversations_partner_cat') else False
                        }).id
            # Update message author
            msg_dict.update({'author_id': author_id})

        # 2. Check for recipients in both 'to' and 'cc'. Can be separated lately to implement custom logic.
        partner_ids = []
            ...
            for email in to.split(","):
                ...
                if not partner:
                    partner = self.env['res.partner']. \
                        create({'name': partner_name if partner_name and len(partner_name) > 0 else
                    email_address.split("@")[0],
                                'email': email_address,
                                'category_id': [
                                    (4, self.env.ref('prt_mail_messages.cetmix_conversations_partner_cat').id)]
                                if self.env.ref('prt_mail_messages.cetmix_conversations_partner_cat') else False
                                })
                """
                Check if Partner is internal user AND has access to Conversations.
        ...
        return super(Conversation,
                     self.with_context(other_partner_ids=other_partner_ids if len(other_partner_ids) > 0 else False,
                                       mail_create_nolog=True)).message_new(msg_dict, custom_values)
    ...
    def message_post(self, *,
                     ...
                     **kwargs):
        res = super(Conversation, self).message_post(body=body, subject=subject, message_type=message_type,
                                                     ...
                                                     subtype=subtype, attachments=attachments, attachment_ids=attachment_ids,
                                                     add_sign=add_sign, record_name=record_name, **kwargs)
        if message_type not in ["comment", "email"]:  # Skip notifications
            return res
        # Add other_partner_ids as followers. We do it here to avoid them being notified on initial message post
