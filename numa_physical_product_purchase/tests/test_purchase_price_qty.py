...
class TestPurchasePriceQty(TransactionCase):
    ...
    def test_01_a_line_created_in_code_is_priced_by_its_magnitude(self):
        """A line created outside the form is priced by its magnitude.

        The four fields were filled only by onchange handlers, and the tax base was
        handed over through `_convert_to_tax_base_line_dict`, a hook core no longer
        calls. So four slabs of 12.5 kg at 3.00/kg were costed at 12.00 rather than
        150.00.
        """
    ...
    def test_12_the_vendor_price_refresh_is_not_suppressed(self):
        ...
        Core has no method of that name in 18.0, so the override sat there shadowing
