...
class PhysicalSaleOrder(models.Model):
    ...
    so_weight = fields.Float('Peso', compute='_compute_weight_volume')
    so_volume = fields.Float('Volumen', compute='_compute_weight_volume')
    ...
    def _compute_weight_volume(self):
        for so in self:
            ...
            for line in so.order_line:
                ...
                so.so_volume += line.product_uom_qty * line.product_id.width * line.product_id.length * line.product_id.height
...
class PhysicalSaleOrderLine(models.Model):
    ...
    product_length_rel = fields.Float(string='Lenght', related='product_id.length', readonly=True)
