...
class TestSalePriceQty(TransactionCase):
    ...
    def test_16_the_order_total_is_the_magnitude_total(self):
        ...
        not from the lines' stored subtotals. The fork of `_compute_amount` this module
        used to carry got the line right and left that hook alone, so an order of four
        slabs of 12.5 kg at 3.00/kg showed 150.00 on its line and 12.00 at the bottom.
        """
        slab = self._product('weight', price=3.0, weight=12.5)
        order = self._order(slab, qty=4.0)
    ...
    def test_12_tax_is_computed_on_the_magnitude(self):
        ...
        which since 18.0 disagreed with the rest of the document: the order's totals
        and the down payment wizard are built from the hook, not from the line.
        """
        ...
        slab = self._product('weight', price=3.0, weight=12.5)
        order = self._order(slab, qty=4.0)
        order.order_line.tax_id = [(6, 0, tax.ids)]
    ...
    def test_15_no_override_of_a_core_method_that_is_gone(self):
        ...
        had anything still called it. `update_prices` is `_recompute_prices` now.
        ...
        it computed taxes through an API the current engine does not have.
