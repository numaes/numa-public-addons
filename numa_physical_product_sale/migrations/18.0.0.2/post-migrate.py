# -*- coding: utf-8 -*-
"""Fill in the sale order lines the onchange never reached.
...
a quotation template, a duplicated order -- kept them at zero. They are computed fields now, but the
...
def migrate(cr, version):
    ...
    cr.execute("""
        SELECT l.id, o.name, bool_and(pt_all.price_base = 'normal')
          ...
          JOIN sale_order_line l_all ON l_all.order_id = o.id AND l_all.display_type IS NULL
         ...
         WHERE l.display_type IS NULL
           ...
           AND COALESCE(l.product_uom_qty, 0) != 0
    ...
    if lines:
        ...
        env.flush_all()
    _logger.info("numa_physical_product_sale: %s lines filled in", len(lines))

    if held:
        _logger.warning(
            "numa_physical_product_sale: %s orders have lines priced by a physical magnitude whose "
