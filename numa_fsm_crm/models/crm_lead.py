...
class CrmLead(models.Model):
    ...
    The lead is NOT the instance: it has one. Up to Odoo 18 it inherited
    ``fsm.instance`` through ``_inherit`` and became the instance, with no field
    of its own. That stopped linearising in Odoo 20: ``fsm.instance`` inherits
    ``mail.thread`` and ``mail.activity.mixin``, which ``crm.lead`` already
    inherits on its own, and C3 requires the most derived class first while the
    base list leaves it last. It went unnoticed in 18.0 because numa_poly rebuilt
    ``__bases__`` by hand and undid the conflict on the way; now that bases are
    declared, Odoo computes the order and the clash surfaces.

    The explicit link says the same thing without fighting the MRO, and it also
    separates two lifetimes that were never really one: a lead can exist without
    a workflow, and its instance can be restarted without touching the lead.
    """
    _name = 'crm.lead'
    _inherit = ['crm.lead']
    ...
    # What the form and the filters used to read off the lead when it was the
    ...
    def _search_has_fsm(self, operator, value):
        ...
        # hand-written mirror of it: a lead with a definition but no instance yet
        # has an empty ``fsm_state``, and a ``not in`` over the traversal would
        # never reach it.
    ...
    def _ensure_fsm_instance(self):
        """Return this lead's instance, creating it if there is not one yet.

        When the lead WAS the instance this was not needed. It is now the only
